...
uncert_samples_class$uncertainty <- uncert_samples$uncertainty

uncert_samples_sf <- terra::vect(
  uncert_samples,
...
terra::writeVector(
  x = uncert_samples_sf,
  filename = sub("\\.rds$", ".gpkg", uncert_file),
  overwrite = TRUE
)
...
tc_bound <- terra::rast("https://restore-plus.s3.us-east-1.amazonaws.com/ext-maps/tc/cer/boundary/tc_30m_cer_2018-01-01_2018-12-31_class_boundary.tif", vsi = TRUE)
names(tc_bound) <- "tc_v1_2018"
...
tc_vals <- terra::extract(tc_bound, points, ID = FALSE)
...
#   dplyr::filter(label == "Silviculture") |>
